...
public class CookMach {
    public static void main(String[] args) {
        ...
        CookMach convert = new CookMach();
        for (int i = 0; i < numberOfTests; i++) {
            src = in.nextInt();
            dst = in.nextInt();
            System.out.println(convert.findNumberOfOperationsToConvert(src, dst));
        }
    }

    private int findNumberOfOperationsToConvert(int input, int desiredNumber) {
        int numberOfOperations = 0;

        // Not possible to convert 0 to any power of two
        if (input == 0 || desiredNumber == 0) {
            return 0;
        }

        if ((desiredNumber & (desiredNumber - 1)) != 0) {
            throw new IllegalArgumentException("Desired number should be a power of 2!");
        }
        ...
        if (input > 1) {
            int closestPowerOfTwo = findClosestPowerOfTwo(input);
            while (input != closestPowerOfTwo) {
                if (input == 1) {
                    break;
                }
                if (input % 2 == 0) {
    ...
    private int findClosestPowerOfTwo(int input) {
        if (input < 2) {
            return 2;
        }
